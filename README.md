...
- Root Directory:
  - 
  - `README.md` - File structure, usage instructions and directions for adding new Cloud Formation Templates for future labs/livecodes.
...
- Sub Directories:
  ...
  -  **REQUIRED**  
     ...
     -   `main-stack.yaml` - A Cloud Formation template to build the main stack for a specific livecode/lab.
  -   **OPTIONAL**
...
Each subdirectory in this repository contains the files needed to create one or more Cloud Formation Stack(s) to make setup easier and more consistent for Ada Developers Academy Cloud labs and live codes. Use the following steps to run each set up:

1. Go to the lab/livecode's `config.conf` file. Here you will find all the specific variables that are needed to run different stages. Use the comments to update variables that need to be changed. Variables that need to be changed often include things like the region, vpc ids and subnets. Any variable that is not explicitly called out as one that should be updated can be left as is.
